...
emit_curated_int() {
  ...
  case "$option" in
    ...
    modify-column|check|add-drop-partition|drop-column|add-column|drop-index|add-index|rename-column|rename-index|optimize|analyze|truncate|recreate-table|grammar-sql)
      ...
      ;;
    vacuum|vacuum-full|checkpoint|create-index-concurrently|reindex|cluster-table|brin-expression-index|create-matview|refresh-matview-concurrently|select-matview|drop-matview|prepared-tx-stress|pg18-not-null-constraint|pg18-temporal-constraints|pg18-partition-ops)
      add_param "$option" "$(rand_int_range 0 50)"
      ;;
